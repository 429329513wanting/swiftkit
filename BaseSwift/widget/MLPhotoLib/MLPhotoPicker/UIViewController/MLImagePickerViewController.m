...
- (void)displayForVC:(__weak UIViewController *)viewController completionHandle:(void (^)(BOOL, NSArray<NSURL *> *, NSArray<UIImage *> *, NSArray<UIImage *> *, NSError *))completionHandle
{
    ...
    [MLPhotoPickerManager manager].navigationController = [[UINavigationController alloc] initWithRootViewController:self];
...
- (void)setupSubviews
{
    ...
    [self.view addSubview:_contentCollectionView = [[MLPhotoPickerCollectionView alloc] initWithFrame:(CGRect){CGPointMake(0, 0), CGSizeMake(self.view.frame.size.width, self.view.frame.size.height - self.navigationController.navigationBar.frame.size.height)}]];
}
