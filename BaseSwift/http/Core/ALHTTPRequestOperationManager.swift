...
open class ALHTTPRequestOperationManager {
    ...
    open func uploadPicture(url urlString:String,
                         ...
                         encodingCompletion: ((SessionManager.MultipartFormDataEncodingResult) -> Void)?)
    {
        ...
        Alamofire.upload(multipartFormData: { (formdata) in
            ...
            let imgData: Data = file["image"] as! Data
            let formater = DateFormatter()
            formater.dateFormat = "yyyyMMddHHmmss"
            let fileName = formater.string(from: Date())+".png"
            
            formdata.append(imgData, withName: "file", fileName:fileName, mimeType: "image/png")
    ...
    ///   - multipartFormData: 文件传入处理
    ///   - encodingCompletion: 结果回调(以枚举返回)
    open func uploadMultiPartForm(url urlString:String,
                            parameter:[String : String]? = [String : String](),
                            file:[String: Data] = [String: Data](),
                            preSetupHandle: ALHTTPManagerSetupHandle? = nil,
                            encodingCompletion: ((SessionManager.MultipartFormDataEncodingResult) -> Void)?)
    {
        ...
        Alamofire.upload(multipartFormData: { (formdata) in
            ...
            for(key,value) in file{
                ...
                if key == "image"{
                    ...
                    formater.dateFormat = "yyyyMMddHHmmss"
                    let fileName = formater.string(from: Date())+".png"
                    formdata.append(value, withName: "file", fileName: fileName, mimeType: "image/png")
                }
            }
